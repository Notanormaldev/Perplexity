...
// ── SEARCH MODAL — Original Claude-style (restored) ──
...
// ── History Panel — side drawer (different from search) ──
...
// ── Model Selector ──
const MODELS=[
  {id:'gemini-2.0-flash',label:'Gemini 2.0 Flash',provider:'Google',icon:'ri-google-line'},
  {id:'gemini-1.5-pro',label:'Gemini 1.5 Pro',provider:'Google',icon:'ri-google-line'},
  {id:'claude-3-5-sonnet',label:'Claude 3.5 Sonnet',provider:'Anthropic',icon:'ri-robot-2-line'},
  {id:'claude-3-haiku',label:'Claude 3 Haiku',provider:'Anthropic',icon:'ri-robot-2-line'},
  {id:'gpt-4o',label:'GPT-4o',provider:'OpenAI',icon:'ri-openai-line'},
  {id:'gpt-4o-mini',label:'GPT-4o Mini',provider:'OpenAI',icon:'ri-openai-line'},
  {id:'llama-3.3-70b',label:'Llama 3.3 70B',provider:'Groq',icon:'ri-flashlight-line'},
  {id:'mistral-large',label:'Mistral Large',provider:'Mistral',icon:'ri-wind-line'},
]
...
const ModelSelector = ({ selectedModel, onSelect }) => {
  ...
  return(
    <div ref={ref} style={{position:'relative'}}>
      ...
      {open&&(
        <div style={{position:'absolute',bottom:'calc(100% + 6px)',left:0,background:'#1e1e1e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:12,padding:'4px',boxShadow:'0 16px 40px rgba(0,0,0,.7)',zIndex:90,minWidth:220,animation:'dropDown .16s ease'}}>
...
const AttachMenu = ({ onAttach }) => {
  ...
  const opts=[
    {icon:'ri-image-line',label:'Image',accept:'image/png,image/jpeg,image/gif,image/webp',color:'#a1a1aa'},
    {icon:'ri-file-pdf-line',label:'PDF Document',accept:'application/pdf',color:'#a1a1aa'},
    {icon:'ri-braces-line',label:'JSON / Text',accept:'.json,.txt,.csv',color:'#a1a1aa'},
  ]
  const [h,setH]=useState(false)
  ...
  return(
    <div ref={ref} style={{position:'relative',flexShrink:0}}>
      ...
      {open&&(
        <div style={{position:'absolute',bottom:'calc(100% + 6px)',left:0,background:'#1e1e1e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:11,padding:'4px',boxShadow:'0 10px 32px rgba(0,0,0,.65)',zIndex:60,minWidth:175,animation:'dropDown .16s ease'}}>
          {opts.map(opt=>(
            <label key={opt.label}
...
const InputBar=({value,onChange,onSubmit,onKeyDown,onAudio,isRecording,textareaRef,disabled,large,attachedFile,onAttach,onRemoveFile,selectedModel,onModelChange,showModel})=>{
  ...
  return(
    <div style={{background:'rgba(255,255,255,0.05)',border:`1px solid ${focused?'rgba(255,255,255,0.13)':'rgba(255,255,255,0.08)'}`,borderRadius:14,transition:'all .18s',overflow:'hidden'}}>
      {/* Attached file pill */}
      {attachedFile&&(
        <div style={{padding:'7px 12px 0',display:'flex'}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'4px 10px',borderRadius:8,background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.09)'}}>
            <i className={attachedFile.name.endsWith('.pdf')?'ri-file-pdf-line':attachedFile.name.endsWith('.json')?'ri-braces-line':'ri-image-line'} style={{fontSize:12,color:'#9ca3af'}}/>
      ...
      {/* Bottom: Model selector (only shown when showModel=true) */}
      {showModel&&(
...
// ── WELCOME SCREEN — Logo → Input → Chips → Suggestions ──
const WelcomeScreen=({onSendMessage,incognito,onToggleIncognito,selectedModel,onModelChange})=>{
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 24px 32px',background:incognito?'#090909':'#0d0d0d',minHeight:'100dvh',transition:'background .3s',position:'relative',overflowY:'auto'}}>
      ...
      {/* 1. Input bar (with model selector inside) */}
        ...
        {CHIPS.map((c,i)=>{
          ...
          return(
            <button key={i} onClick={()=>onSendMessage(c.text)}
              ...
              <i className={c.icon} style={{fontSize:13}}/>{c.text}
            </button>
          )
        })}
      </div>
      ...
      <div style={{width:'100%',maxWidth:600}}>
        {SUGGESTIONS.map((s,i)=><SuggestionRow key={i} text={s} onClick={onSendMessage}/>)}
      </div>
    </div>
  )
}

// ── Chat Input Area — chips below inputbar ──
const CHAT_CHIPS=[
          ...
          {CHAT_CHIPS.map((c,i)=>{
            ...
            return(
              <button key={i} onClick={()=>setValue(c.text+' ')}
                onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
                style={{display:'flex',alignItems:'center',gap:6,padding:'5px 12px',borderRadius:10,background:h?'rgba(255,255,255,0.07)':'rgba(255,255,255,0.04)',border:`1px solid ${h?'rgba(255,255,255,0.1)':'rgba(255,255,255,0.06)'}`,color:h?'#ececec':'#9ca3af',fontSize:12,cursor:'pointer',whiteSpace:'nowrap',transition:'all .13s',flexShrink:0}}>
...
function Dashboard(){
  ...
  const onSend=content=>handlegenraterespons({message:content,chatid:incognito?null:currentchatid||null})
